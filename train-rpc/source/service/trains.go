...
import (
	...
	"errors"
	"fmt"
	trainsRpc "github.com/yongsuha/goTrain/train-proto/trains"
...
// ToDo 应该可以考虑用协程吧
func (t *TrainsService) UpdateTrain(ctx context.Context, req *trainsRpc.UpdateTrainReq) error {
	tag := "TrainsService:UpdateTrain"
	if req.TrainId == 0 {
		return errors.New("TrainId is empty")
	}
	// 先获取到要修改车次的信息
...
func (t *TrainsService) DelTrain(ctx context.Context, req *trainsRpc.DelTrainReq) error {
	...
	if trainMessage == nil {
		return errors.New("the train message is empty")
	}
